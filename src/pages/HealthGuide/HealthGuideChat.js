...
export default function HealthGuideChat(currentUserId) {
  ...
  const update = (
    ...
    currentCache.writeQuery({
      ...messagesQueryParams,
      data: {
        chatRoom: { ...chatRoom.chatRoom, messages: [...chatRoom.chatRoom.messages, message] },
      },
    })
    if (errors) {
  ...
  return (
    <div>
      <Button variant="contained" onClick={() => navigate(`/health_guide?${searchParams}`)}>Back to Dashboard</Button>
      <ChatRoom 
